	...
	class BaseOutlineGenerator {
		...
		_addStyles() {
			...
			: root {--outline - bg - light: rgba(255, 255, 255, 0.75); --outline - bg - dark: rgba(30, 30, 30, 0.75); --outline - hover - bg - light: rgba(240, 240, 240, 0.8); --outline - hover - bg - dark: rgba(50, 50, 50, 0.9); --outline - text - light: #333; --outline - text - dark: #f1f1f1; --outline - active - color: #00A9FF; --outline - border - light: rgba(0, 0, 0, 0.1); --outline - border - dark: rgba(255, 255, 255, 0.15); --outline - shadow: 0 8px 32px 0 rgba(31, 38, 135, 0.17);
				...
				border - left - color: var (--outline - active - color);
				...
				height: 48px;
				border - radius: 50 % ;
				background: linear - gradient(135deg, #00A9FF, #1C82AD);
				color: white;
				display: flex;
				align - items: center;
		...
		_waitForContent(callback) {
			...
			this.show();
			this._setItemsContainerMessage('正在加载大纲...');
			let interval,
				timeout;
			const cleanup = () = > {
